...
slug = "1155"
aliases = ["01155", "ERCS/erc-1155", "EIPS/eip-1155"]
...
[extra]
proposal_id = "ERC-1155"
requires = ["@/00165.md"]
discussions_to = "https://github.com/ethereum/EIPs/issues/1155"
category = "ERC"
status = "Final"
number = 1155
type = "Standards Track"
...
email = "witek@enjin.io"
name = "Witek Radomski"

[[extra.author_details]]
email = "ac0dem0nk3y@gmail.com"
name = "Andrew Cooke"

[[extra.author_details]]
email = "pc@horizongames.net"
github = "phabc"
name = "Philippe Castonguay"

[[extra.author_details]]
email = "james@turing-complete.com"
name = "James Therien"
...
* An example:
  ...
  1. `myTransferFrom` updates the balances for `_from` and `_to` addresses for all `_ids` and `_values`.
  1. `myTransferFrom` emits `TransferBatch` with the details of what was transferred from address `_from` to address `_to`.
  1. `myTransferFrom` checks if `_to` is a contract address and determines that it is so (if not, then the transfer can be considered successful).
  1. `myTransferFrom` calls `onERC1155BatchReceived` on `_to` and it reverts or returns an unknown value (if it had returned `bytes4(keccak256("onERC1155BatchReceived(address,address,uint256[],uint256[],bytes)"))` the transfer can be considered successful).
  1. At this point `myTransferFrom` SHOULD revert the transaction immediately as receipt of the token(s) was not explicitly accepted by the `onERC1155BatchReceived` function.
  1. If however `myTransferFrom` wishes to continue it MUST call `supportsInterface(0x4e2312e0)` on `_to` and if it returns the constant value `true` the transaction MUST be reverted, as it is now known to be a valid receiver and the previous acceptance step failed.
  ...
  1. If the above call to `supportsInterface(0x4e2312e0)` on `_to` reverts or returns a value other than the constant value `true` the `myTransferFrom` function MAY consider this transfer successful.
...
The optional `ERC1155Metadata_URI` extension can be identified with the [ERC-165 Standard Interface Detection](@/00165.md).
...
The function `setApprovalForAll` allows an operator to manage one's entire set of tokens on behalf of the approver. To permit approval of a subset of token IDs, an interface such as [ERC-1761 Scoped Approval Interface](@/01761.md) is suggested.
...
1. If the function call succeeds and the return value is the constant value `true` the implementation proceeds as a regular ERC-1155 implementation, with the call(s) to the `onERC1155Received` or `onERC1155BatchReceived` hooks and rules associated.
1. If the function call fails or the return value is NOT the constant value `true` the implementation can assume the recipient contract is not an `ERC1155TokenReceiver` and follow its other standard's rules for transfers.
...
* [ERC-165 Standard Interface Detection](@/00165.md)
* [ERC-1538 Transparent Contract Standard](@/01538.md)

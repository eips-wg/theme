...
$DefaultActiveRepoUrl = "https://github.com/eips-wg/EIPs.git"
...
function Assert-ActiveRepoManifest {
    param(
        [string]$ActiveRepoRoot,
        [bool]$ActiveRepoExplicit
    )

    $manifestPath = Join-Path -Path $ActiveRepoRoot -ChildPath "Build.toml"
    if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
        return
    }

    if ($ActiveRepoExplicit) {
        Die "configured ACTIVE_REPO_ROOT does not contain Build.toml: $ActiveRepoRoot. Point ACTIVE_REPO_ROOT at a manifest-bearing EIPs or ERCs checkout."
    }

    Die "default EIPs checkout at $ActiveRepoRoot does not contain Build.toml. The eips-wg/EIPs default branch is awaiting the Build.toml rollout; set ACTIVE_REPO_ROOT to a manifest-bearing EIPs or ERCs checkout until it is available."
}
...
Assert-ActiveRepoManifest -ActiveRepoRoot $ActiveRepoRoot -ActiveRepoExplicit $ActiveRepoExplicit
